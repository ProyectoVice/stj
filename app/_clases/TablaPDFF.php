...
class TablaPDFF extends Fpdf{
	...
	function Header(){
            ...
            $this->Cell(5,6,':',0,0,'L');
            $this->Cell(75,6,$this->carga->semestre,0,0,'L');
            $this->Cell(20,6,utf8_decode('Año'),0,0,'L');
            $this->Cell(5,6,':',0,0,'L');
        $txt = utf8_decode($this->carga->anio);
            $this->Cell(75,6, $txt,0,0,'L');
            $this->Ln();
            $this->Ln();
...
// Tabla coloreada
    ...
    function cabeceraLineaDeTeimpo($header,$w,$size){
        ...
        for($i=0;$i<count($header);$i++)
            list($big_y, $x)=$this->MultiCellAndPoss($w[$i], 2, $header[$i], 1, 'C', true, $this->GetX(), $this->GetY(), $this->GetY(), 2, 2);

...
@section('titulo','Calendario Escuela')
@section('activacion')
@endsection
@section('estilos')
...
<ul class="breadcrumb">
  ...
  <li class=""><a href="{{ route('academico.cal_escuela.index') }}"></a></li>
...
@section('contenido')
  <div class="row">
    <div class="col-sm-12">
      <h3><u> Escuela</u>: {{ $proyecto->titulo }}</h3><hr><br>
      {{ csrf_field() }}
    </div>
          ...
          <div id="testmodal" style="padding: 5px 20px;">
            <form id="antoform" class="form-horizontal calender" role="form">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-9">
                  <textarea class="form-control" style="height:55px;" id="txtresponsable" name="descr"  required="true"></textarea>
                </div>
              </div>
              <div class="form-group">
...
var myTable=$('#dynamic-table').DataTable( {
  "processing": true,
  "serverSide": true,
  "ajax": '{!! route('academico.cal_escuela.tabla',$proyecto->id) !!}',
...
dayClick: function(date, jsEvent, view) {
  ...
  events: '{!! route('academico.cal_escuela.data',$proyecto->id) !!}',
  //Click en un evento
...
//Agregar un evento
  ...
  $.ajax({ 
    url: '{{ route('academico.cal_escuela.new') }}',
    type: 'POST',
    data: NuevoEvento,
...
//Eliminar un evento
  ...
  //capturamos los datos
    recolectarDatos();
    $.ajax({ 
      url: '{{ route('academico.cal_escuela.del') }}',
      type: 'POST',
      data: NuevoEvento,
      success: function (data) {
        $('#calendar').fullCalendar('refetchEvents');
...
//Actualizar un evento
  ...
  //capturamos los datos
    recolectarDatos();
    $.ajax({ 
      url: '{{ route('academico.cal_escuela.act') }}',
      type: 'POST',
      data: NuevoEvento,
      success: function (data) {
        $('#calendar').fullCalendar('refetchEvents');
